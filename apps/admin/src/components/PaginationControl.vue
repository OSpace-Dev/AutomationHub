...
<template>
  <nav class="pagination-bar" aria-label="分页导航">
    ...
    <div class="pagination-actions">
      <button class="icon-button" type="button" title="上一页" aria-label="上一页" :disabled="page <= 1" @click="changePage(page - 1)">
        <ChevronLeft :size="15" aria-hidden="true" />
      </button>
      <label class="pagination-jump">
        <span class="sr-only">跳转页码</span>
        <input v-model="pageDraft" type="number" inputmode="numeric" min="1" :max="totalPages" aria-label="跳转页码" @keydown.enter.prevent="jumpToDraftPage" />
      </label>
      <button class="secondary-button pagination-jump-button" type="button" :disabled="!canJump" @click="jumpToDraftPage">跳转</button>
      <button class="icon-button" type="button" title="下一页" aria-label="下一页" :disabled="page >= totalPages" @click="changePage(page + 1)">
