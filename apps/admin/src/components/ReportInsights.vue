...
const overviewFacts = computed(() => {
  const facts = [`${props.insights.metrics.project_count} 个项目`, `${props.insights.metrics.category_count} 个分类`];
  if (props.insights.metrics.stars_today !== undefined)
    facts.push(`今日 +${formatNumber(props.insights.metrics.stars_today)} Star`);
...
const trendByProject = computed(() => {
  ...
  for (const project of props.insights.trends.new_entries)
    values.set(normalizeUrl(project.project_url), { text: "新上榜", tone: "accent" });
  for (const project of props.insights.trends.rising_entries)
    values.set(normalizeUrl(project.project_url), { text: `排名 ↑${project.rank_change ?? 0}`, tone: "positive" });
  for (const project of props.insights.trends.falling_entries)
    values.set(normalizeUrl(project.project_url), {
      text: `排名 ↓${Math.abs(project.rank_change ?? 0)}`,
      tone: "negative"
    });
  return values;
});
...
function projectSummary(project: ReportProjectInsight) {
  const purpose = cleanSummarySegment(
    project.purpose?.trim() || project.description?.trim() || `${project.name} 的项目说明暂未完整采集。`
  );
...
function projectTags(project: ReportProjectInsight): ProjectTag[] {
  ...
  if (project.stars_today !== undefined)
    tags.push({ text: `今日 +${formatNumber(project.stars_today)}`, tone: "accent" });
  ...
  <section class="report-insights" aria-label="日报内容">
    <section class="report-overview-card">
      ...
      <div class="report-overview-facts" aria-label="今日辅助数据">
        ...
        <span v-if="insights.trends.has_comparison && insights.trends.comparison_date"
          >对比 {{ insights.trends.comparison_date }}</span
        >
      </div>
    </section>
        ...
        <header class="report-category-header">
          <div class="report-category-title">
            ...
            <div>
              ...
              <span>{{ category.project_count }} 个项目</span>
            </div>
          </div>
          <span v-if="categoryMeta(category.stars_today)" class="report-category-meta">{{
            categoryMeta(category.stars_today)
          }}</span>
        </header>
          ...
          <article v-for="project in category.projects" :key="project.project_url" class="report-project-card">
            ...
            <footer class="report-project-tags" aria-label="项目辅助数据">
              <span
                ...
                :key="`${project.project_url}-${tag.text}`"
                :data-tone="tag.tone"
                >{{ tag.text }}</span
              >
            </footer>
