...
<template>
  <section class="runs-workspace" aria-label="采集批次与项目结果">
    <aside class="runs-sidebar-panel">
      ...
      <PaginationControl v-if="runs.length" :page="pagination.runs.page" :total-pages="pagination.runs.total_pages" :total="pagination.runs.total" item-label="个批次" @change="changePage('runs', $event)" />
    </aside>
    ...
    <section class="results-panel" aria-live="polite">
      ...
      <PaginationControl v-if="items.length" :page="pagination.items.page" :total-pages="pagination.items.total_pages" :total="pagination.items.total" item-label="个项目" @change="changePage('items', $event)" />
    </section>
  </section>
</template>
