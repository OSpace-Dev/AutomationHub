  ...
  <section class="channels-workspace">
    <aside class="channel-list-panel">
      <div class="workspace-panel-header">
        ...
        <button class="icon-button" type="button" title="新建 Telegram Bot" aria-label="新建 Telegram Bot" @click="newChannel"><Plus :size="17" /></button>
      </div>
      ...
      <div v-else class="channel-list">
        <button v-for="channel in channels" :key="channel.id" type="button" class="channel-list-item" :class="{ selected: selectedChannel?.id === channel.id }" @click="selectChannel(channel)">
    ...
    <section class="channel-editor-panel">
      ...
      <div class="channel-editor-scroll">
        ...
        <section v-if="selectedChannel" class="target-section">
          ...
          <div v-else-if="chats.length" class="chat-discovery-list">
            <div v-for="chat in chats" :key="chat.id" class="chat-discovery-row">
              ...
              <div class="chat-row-actions">
                <button class="text-button" type="button" @click="selectChat(chat)"><Plus :size="13" />{{ chat.already_configured ? '重新填写' : '选择' }}</button>
          ...
          <div class="configured-targets">
            <div class="configured-targets-heading"><div><span class="eyebrow">CONFIGURED TARGETS</span><h3>已添加目标</h3></div><button class="secondary-button" type="button" @click="newTarget"><Plus :size="15" /><span>手动添加</span></button></div>
            ...
            <div v-else class="target-list">
              <div v-for="target in targets" :key="target.id" class="target-row">
                ...
                <div class="target-row-actions"><button class="text-button" type="button" @click="editTarget(target)">编辑</button><button class="text-button" type="button" :disabled="testingTargetId === target.id" @click="testTarget(target)"><RefreshCw v-if="testingTargetId === target.id" :size="13" class="spinning" /><Send v-else :size="13" />测试推送</button><button class="text-button danger-text" type="button" @click="deleteTarget(target)">删除</button></div>
              </div>
            </div>
          </div>

          <form class="target-form" @submit.prevent="saveTarget">
            <div class="target-form-heading"><span class="eyebrow">{{ editingTargetId ? 'EDIT TARGET' : 'NEW TARGET' }}</span><h3>{{ editingTargetId ? '编辑推送目标' : '添加推送目标' }}</h3></div>
            <div class="target-form-fields">
              <label class="target-field"><span class="target-field-label">目标名称</span><input v-model="targetForm.name" type="text" placeholder="例如：日报群" required /></label>
              <label class="target-field"><span class="target-field-label">chat_id</span><input v-model="targetForm.chatId" type="text" placeholder="例如：-1001234567890" required /></label>
            </div>
            <p class="target-form-help"><Info :size="14" aria-hidden="true" /><span>也可以从上方读取的会话中直接选择。</span></p>
            <div class="target-form-footer">
              <label class="checkbox-label"><input v-model="targetForm.enabled" type="checkbox" /><span>启用该目标</span></label>
              <div class="target-form-actions"><button v-if="editingTargetId" class="secondary-button" type="button" @click="newTarget">取消编辑</button><button class="primary-button" type="submit" :disabled="targetActionLoading"><RefreshCw v-if="targetActionLoading" :size="15" class="spinning" /><Save v-else :size="15" /><span>{{ targetActionLoading ? '保存中' : '保存目标' }}</span></button></div>
            </div>
          </form>
        </section>
      </div>
