...
const hasScheduleFilters = computed(() =>
  Boolean(scheduleStatus.value || scheduleDeviceFilter.value || scheduleRecurrence.value)
);
const selectedIsTask = computed(() => Boolean(selectedRecord.value && "business_date" in selectedRecord.value));
const selectedSchedule = computed(() =>
  selectedRecord.value && !selectedIsTask.value ? (selectedRecord.value as TaskSchedule) : null
);
async function submitTask() {
  if (await createTask()) createDrawerOpen.value = false;
}
function openRecord(record: Task | TaskSchedule) {
  selectedRecord.value = record;
  detailDrawerOpen.value = true;
}
async function cancelSelectedTask(task: Task) {
  if (await cancelTask(task)) detailDrawerOpen.value = false;
}
async function cancelSelectedSchedule(schedule: TaskSchedule) {
  if (await cancelSchedule(schedule)) detailDrawerOpen.value = false;
}
function activateRecord(event: KeyboardEvent, record: Task | TaskSchedule) {
    ...
    <section class="data-section task-center-card" aria-label="任务列表">
      ...
      <section class="compact-filter-bar" aria-label="任务筛选">
        ...
        <button
          ...
          type="button"
          :disabled="activeList === 'tasks' ? !hasTaskFilters : !hasScheduleFilters"
          @click="resetTaskFilters(activeList)"
        >
      ...
      >
        ...
        <div v-else-if="!tasks.length" class="empty-state compact">
          <Activity :size="26" aria-hidden="true" /><strong>没有符合条件的任务</strong
          ><span>调整筛选条件，或从上方下发新的采集任务。</span>
        </div>
          ...
          <table>
            ...
            <tbody>
              <tr
                ...
                @keydown="activateRecord($event, task)"
              >
                <td>{{ formatTime(task.created_at) }}</td>
                <td class="mono-cell">{{ task.device_id }}</td>
                <td>{{ task.business_date }}</td>
                <td>
                  <span class="status" :data-status="task.status">{{ statusLabel(task.status) }}</span>
                </td>
                <td class="mono-cell">{{ task.run_id || "—" }}</td>
                <td>
                  <button
                    class="row-detail-button"
                    type="button"
                    aria-label="查看任务详情"
                    @click.stop="openRecord(task)"
                  >
                    <Eye :size="15" />
                  </button>
                </td>
        ...
        <PaginationControl
          ...
          :total-pages="pagination.tasks.total_pages"
      ...
      <div v-else id="schedules-panel" class="task-list-panel" role="tabpanel" aria-label="预约计划列表">
        ...
        <div v-else-if="!schedules.length" class="empty-state compact">
          <Repeat2 :size="26" aria-hidden="true" /><strong>没有符合条件的预约计划</strong
          ><span>调整筛选条件，或选择单次预约、每日模式创建计划。</span>
        </div>
        <div v-else class="table-scroll">
          <table>
            <thead>
            ...
            <tbody>
              <tr
                ...
                class="clickable-row"
                tabindex="0"
                @click="openRecord(schedule)"
                @keydown="activateRecord($event, schedule)"
              >
                <td>{{ schedule.recurrence === "daily" ? "每日" : "单次" }}</td>
                <td class="mono-cell">{{ schedule.device_id }}</td>
                <td>
                  <span class="status" :data-status="schedule.status">{{ statusLabel(schedule.status) }}</span>
                </td>
                <td>{{ formatTime(schedule.next_run_at || schedule.start_at) }}</td>
                ...
                <td>
                  <button
                    ...
                    type="button"
                    aria-label="查看计划详情"
                    @click.stop="openRecord(schedule)"
                  >
                    <Eye :size="15" />
                  </button>
                </td>
        ...
        <PaginationControl
          ...
          :page="pagination.schedules.page"
          :total-pages="pagination.schedules.total_pages"
          :total="pagination.schedules.total"
          item-label="项计划"
          @change="changePage('schedules', $event)"
        />
      </div>
    </section>
    ...
    >
      <div class="drawer-form">
        <div class="drawer-intro">
          ...
          <div><strong>选择任务执行方式</strong><span>创建成功后会保留当前列表筛选和分页位置。</span></div>
        </div>
        <div class="mode-segment drawer-mode" role="group" aria-label="任务模式">
          <button type="button" :class="{ active: taskMode === 'immediate' }" @click="taskMode = 'immediate'">
            立即</button
      ...
      <template #footer
        ><div class="drawer-footer">
          <el-button :disabled="creatingTask" @click="createDrawerOpen = false">取消</el-button
          ><el-button type="primary" :loading="creatingTask" :disabled="!taskDeviceId" @click="submitTask"
            ><Clock3 v-if="taskMode !== 'immediate' && !creatingTask" :size="16" /><Send
              v-else-if="!creatingTask"
              :size="16"
            />{{ taskMode === "immediate" ? "立即下发" : "创建计划" }}</el-button
          >
        </div></template
      >
    </el-drawer>
      ...
      <div v-if="selectedRecord" class="detail-stack">
        <div class="detail-hero">
          <span class="drawer-icon"
            ><Activity v-if="selectedIsTask" :size="18" /><CalendarClock v-else :size="18"
          /></span>
          <div>
            <strong>{{
              selectedIsTask ? "采集任务" : selectedSchedule?.recurrence === "daily" ? "每日计划" : "单次计划"
            }}</strong
        ...
        <dl class="detail-list">
          <div>
            <dt>设备</dt>
            <dd>{{ selectedRecord.device_id }}</dd>
          </div>
          <div>
            <dt>状态</dt>
            <dd>{{ statusLabel(selectedRecord.status) }}</dd>
          </div>
          <template v-if="selectedIsTask"
            ><div>
              <dt>业务日期</dt>
              <dd>{{ (selectedRecord as Task).business_date }}</dd>
            </div>
            <div>
              <dt>创建时间</dt>
              <dd>{{ formatTime(selectedRecord.created_at) }}</dd>
            </div>
            <div>
              <dt>批次</dt>
              <dd>{{ (selectedRecord as Task).run_id || "尚未生成" }}</dd>
            </div>
            <div>
              <dt>错误</dt>
              <dd>{{ (selectedRecord as Task).error_code || "无" }}</dd>
            </div></template
          ><template v-else
            ><div>
              <dt>首次执行</dt>
              <dd>{{ formatTime((selectedRecord as TaskSchedule).start_at) }}</dd>
            </div>
            <div>
              <dt>下次执行</dt>
              <dd>
                {{
                  formatTime((selectedRecord as TaskSchedule).next_run_at || (selectedRecord as TaskSchedule).start_at)
                }}
              </dd>
            </div>
            <div>
              <dt>时区</dt>
              <dd>{{ (selectedRecord as TaskSchedule).time_zone }}</dd>
            </div></template
          >
        </dl>
      </div>
      <template #footer
        ><div v-if="selectedRecord" class="drawer-footer">
          <el-button
            v-if="selectedIsTask && ['pending', 'running', 'paused'].includes(selectedRecord.status)"
            type="danger"
            :loading="cancellingTaskId === selectedRecord.id"
            @click="cancelSelectedTask(selectedRecord as Task)"
            ><Ban :size="15" />取消任务</el-button
          ><el-button
            v-else-if="!selectedIsTask && selectedRecord.status === 'active'"
            type="danger"
            :loading="cancellingScheduleId === selectedRecord.id"
            @click="cancelSelectedSchedule(selectedRecord as TaskSchedule)"
            ><Ban :size="15" />停用计划</el-button
