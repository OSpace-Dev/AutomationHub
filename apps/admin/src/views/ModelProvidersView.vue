...
<template>
  <section class="models-workspace">
    ...
    <section class="model-editor-panel">
      ...
      <form class="model-form" @submit.prevent="saveProvider">
        <label>配置名称<input v-model="providerForm.name" type="text" placeholder="例如：生产日报模型" required /></label>
        <label>Base URL<input v-model="providerForm.baseUrl" type="url" placeholder="https://api.example.com/v1" required /><small>填写兼容 API 根路径，服务端会追加 /models 和 /chat/completions。</small></label>
        <label>API Key<div class="secret-input"><KeyRound :size="16" /><input v-model="providerForm.apiKey" type="password" autocomplete="new-password" :required="!selectedProvider?.api_key_configured" :placeholder="selectedProvider?.api_key_configured ? `已保存 ${selectedProvider.api_key_hint}，留空不替换` : '输入 API Key'" /></div><small v-if="selectedProvider?.api_key_configured">已保存 {{ selectedProvider.api_key_hint }}；接口和页面均不会回显完整密钥。</small></label>
        <div class="model-picker">
          <label>模型名称<input v-model="providerForm.selectedModel" type="text" list="available-models" placeholder="拉取后选择，或手动输入模型标识" required /><datalist id="available-models"><option v-for="model in availableModels" :key="model.id" :value="model.id">{{ model.name }}</option></datalist></label>
          <button class="secondary-button" type="button" :disabled="modelFetchLoading || !providerForm.baseUrl" @click="fetchModels"><RefreshCw :size="15" :class="{ spinning: modelFetchLoading }" /><span>{{ modelFetchLoading ? '拉取中' : '拉取模型' }}</span></button>
        </div>
