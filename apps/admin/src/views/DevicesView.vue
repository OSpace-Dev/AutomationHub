...
<template>
  <div class="device-workspace list-first-workspace">
    <section class="data-section devices-section"><div class="section-header"><div><h2>设备列表</h2><p>查看授权状态、最近心跳和本地待上传队列</p></div><div class="section-actions"><span>{{ devices.length }} 台 / 共 {{ pagination.devices.total }} 台</span><el-button type="primary" @click="authorizationDrawerOpen = true"><KeyRound :size="16" />授权管理</el-button></div></div>
      <div v-if="loading" class="loading-state" aria-live="polite"><RefreshCw :size="20" class="spinning" aria-hidden="true" /><span>正在读取设备</span></div><div v-else-if="!devices.length" class="empty-state"><Monitor :size="28" aria-hidden="true" /><strong>暂无采集设备</strong><span>生成授权码并在扩展中完成连接后，设备会出现在这里。</span><el-button type="primary" @click="authorizationDrawerOpen = true"><Plus :size="16" />添加授权</el-button></div>
    ...
    <el-drawer v-model="authorizationDrawerOpen" title="插件授权管理" size="min(620px, 100%)" class="admin-drawer authorization-drawer">
      <div class="authorization-card drawer-card">
      <div class="drawer-intro"><span class="drawer-icon"><KeyRound :size="18" /></span><div><strong>一码一设备</strong><span>明文只在生成后显示一次，已使用授权删除时会同步撤销设备。</span></div></div>
