...
async function createTask() { if (!taskDeviceId.value) { errorMessage.value = "请先选择一个有效设备。"; return false; } creatingTask.value = true; errorMessage.value = ""; try { if (taskMode.value === "immediate") { await apiFetch<Task>("/api/v1/admin/tasks", { method: "POST", headers: { "idempotency-key": crypto.randomUUID() }, body: JSON.stringify({ device_id: taskDeviceId.value, type: "capture_trending", business_date: taskBusinessDate.value }) }); pagination.tasks.page = 1; } else { const startAt = new Date(taskStartAt.value); if (!Number.isFinite(startAt.getTime())) throw new Error("请选择有效的执行时间。"); await apiFetch<TaskSchedule>("/api/v1/admin/schedules", { method: "POST", headers: { "idempotency-key": crypto.randomUUID() }, body: JSON.stringify({ device_id: taskDeviceId.value, type: "capture_trending", recurrence: taskMode.value, start_at: startAt.toISOString(), time_zone: taskTimeZone.value }) }); pagination.schedules.page = 1; taskStartAt.value = defaultStartAt(); } await refreshView("tasks"); return true; } catch (error) { errorMessage.value = error instanceof Error ? error.message : "任务创建失败。"; return false; } finally { creatingTask.value = false; } }
async function cancelTask(task: Task) { if (!["pending", "running", "paused"].includes(task.status)) return false; if (!window.confirm(`确认取消任务“${task.id}”吗？插件将在下一次心跳时停止执行。`)) return false; cancellingTaskId.value = task.id; errorMessage.value = ""; try { await apiFetch<Task>(`/api/v1/admin/tasks/${encodeURIComponent(task.id)}:cancel`, { method: "POST" }); await refreshView("tasks"); return true; } catch (error) { errorMessage.value = error instanceof Error ? error.message : "任务取消失败。"; return false; } finally { cancellingTaskId.value = ""; } }
async function cancelSchedule(schedule: TaskSchedule) { if (schedule.status !== "active" || !window.confirm("确认停用该计划吗？已经生成的任务不会被删除。")) return false; cancellingScheduleId.value = schedule.id; errorMessage.value = ""; try { await apiFetch<TaskSchedule>(`/api/v1/admin/schedules/${encodeURIComponent(schedule.id)}`, { method: "DELETE" }); await refreshView("tasks"); return true; } catch (error) { errorMessage.value = error instanceof Error ? error.message : "计划停用失败。"; return false; } finally { cancellingScheduleId.value = ""; } }
