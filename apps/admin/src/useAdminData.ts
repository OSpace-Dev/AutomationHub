...
const monitoringDevices = ref<Device[]>([]);
const taskDevices = ref<Device[]>([]);
const authorizations = ref<DeviceAuthorization[]>([]);
...
const authorizationExpiresIn = ref<"24h" | "7d" | "30d" | "never">("24h");
const createdAuthorizationCode = ref("");
const creatingAuthorization = ref(false);
const deletingAuthorizationId = ref("");
const taskMode = ref<"immediate" | "once" | "daily">("immediate");
const taskStartAt = ref(defaultStartAt());
const taskTimeZone = ref(Intl.DateTimeFormat().resolvedOptions().timeZone || "UTC");
const cancellingScheduleId = ref("");
const emptyMeta = (): PageMeta => ({ total: 0, page: 1, page_size: 20, total_pages: 1 });
const pagination = reactive({
  runs: emptyMeta(),
  devices: emptyMeta(),
  authorizations: emptyMeta(),
  tasks: emptyMeta(),
  schedules: emptyMeta(),
  logs: emptyMeta(),
  items: emptyMeta()
...
const metrics = computed(() => ({
  runs: pagination.runs.total,
  projects: runs.value.reduce((total, run) => total + run.itemCount, 0),
  success: runs.value.reduce((total, run) => total + run.successCount, 0),
  failed: runs.value.reduce((total, run) => total + run.failureCount, 0),
  onlineDevices: devices.value.filter(isDeviceOnline).length,
  monitoringOnlineDevices: monitoringDevices.value.filter(isDeviceOnline).length,
  pendingTasks: tasks.value.filter((task) => task.status === "pending" || task.status === "running").length
}));
const connectionLabel = computed(
  () => ({ disconnected: "未连接", connecting: "连接中", online: "服务在线", error: "连接异常" })[connectionState.value]
);
...
export function useAdminData() {
  return {
    ...
    createdAuthorizationCode,
    creatingAuthorization,
    deletingAuthorizationId,
    taskMode,
    taskStartAt,
    taskTimeZone,
...
async function connect(view: AdminView) {
  return login(view);
}
async function changeDate(scope: "runs" | "tasks") {
  if (scope === "runs") {
    pagination.runs.page = 1;
    clearRunSelection();
    await refreshView("runs");
  } else {
    await applyTaskFilters("tasks");
  }
}
async function applyTaskFilters(kind: "tasks" | "schedules") {
  pagination[kind].page = 1;
  await refreshView("tasks");
}
async function resetTaskFilters(kind: "tasks" | "schedules") {
  if (kind === "tasks") {
    tasksDate.value = "";
    taskStatus.value = "";
    taskDeviceFilter.value = "";
  } else {
    scheduleStatus.value = "";
    scheduleDeviceFilter.value = "";
    scheduleRecurrence.value = "";
  }
...
async function refreshRuns(options: RefreshOptions) {
  const response = await apiFetch<Run[]>(
    pagePath(`/api/v1/admin/runs?date=${encodeURIComponent(runsDate.value)}`, pagination.runs)
  );
  ...
  if (!refreshedRun) {
    if (!options.background) clearRunSelection();
    return;
  }
...
async function refreshDevices() {
  ...
  Object.assign(
    pagination.authorizations,
    authorizationResponse.meta ?? localMeta(authorizationResponse.data, pagination.authorizations)
  );
}
async function refreshTasks() {
  ...
  const schedulePath = scheduleQuery.size
    ? `/api/v1/admin/schedules?${scheduleQuery.toString()}`
  ...
  if (!taskDeviceId.value)
    taskDeviceId.value = taskDevices.value.find((device) => device.status === "active")?.id ?? "";
}
...
async function selectRun(run: Run, options: SelectRunOptions = {}) {
  ...
  try {
    const response = await apiFetch<Item[]>(
      pagePath(`/api/v1/admin/runs/${encodeURIComponent(run.id)}/items`, pagination.items)
    );
...
function clearRunSelection() {
  ...
  Object.assign(pagination.items, emptyMeta());
}
async function createAuthorization() {
  creatingAuthorization.value = true;
  createdAuthorizationCode.value = "";
  errorMessage.value = "";
  try {
    const response = await apiFetch<{ authorization: DeviceAuthorization; code: string }>(
      "/api/v1/admin/authorizations",
      { method: "POST", body: JSON.stringify({ expires_in: authorizationExpiresIn.value }) }
    );
    createdAuthorizationCode.value = response.data.code;
    pagination.authorizations.page = 1;
    await refreshView("devices");
    return true;
  } catch (error) {
    errorMessage.value = error instanceof Error ? error.message : "授权创建失败。";
    return false;
  } finally {
    creatingAuthorization.value = false;
  }
}
async function deleteAuthorization(authorization: DeviceAuthorization) {
  if (
    !window.confirm(
      authorization.device_id
        ? "删除该授权会同时撤销已连接的插件，历史数据仍会保留。确认继续吗？"
        : "确认删除该未使用授权吗？"
    )
  )
    return;
  deletingAuthorizationId.value = authorization.id;
  errorMessage.value = "";
  try {
    await apiFetch(`/api/v1/admin/authorizations/${encodeURIComponent(authorization.id)}`, { method: "DELETE" });
    if (createdAuthorizationCode.value.endsWith(authorization.code_hint)) createdAuthorizationCode.value = "";
    await refreshView("devices");
  } catch (error) {
    errorMessage.value = error instanceof Error ? error.message : "授权删除失败。";
  } finally {
    deletingAuthorizationId.value = "";
  }
}
async function copyAuthorizationCode() {
  if (!createdAuthorizationCode.value) return;
  await navigator.clipboard.writeText(createdAuthorizationCode.value);
}
async function createTask() {
  if (!taskDeviceId.value) {
    errorMessage.value = "请先选择一个有效设备。";
    return false;
  }
  creatingTask.value = true;
  errorMessage.value = "";
  try {
    if (taskMode.value === "immediate") {
      await apiFetch<Task>("/api/v1/admin/tasks", {
        method: "POST",
        headers: { "idempotency-key": crypto.randomUUID() },
        body: JSON.stringify({
          device_id: taskDeviceId.value,
          type: "capture_trending",
          business_date: taskBusinessDate.value
        })
      });
      pagination.tasks.page = 1;
    } else {
      const startAt = new Date(taskStartAt.value);
      if (!Number.isFinite(startAt.getTime())) throw new Error("请选择有效的执行时间。");
      await apiFetch<TaskSchedule>("/api/v1/admin/schedules", {
        method: "POST",
        headers: { "idempotency-key": crypto.randomUUID() },
        body: JSON.stringify({
          device_id: taskDeviceId.value,
          type: "capture_trending",
          recurrence: taskMode.value,
          start_at: startAt.toISOString(),
          time_zone: taskTimeZone.value
        })
      });
      pagination.schedules.page = 1;
      taskStartAt.value = defaultStartAt();
    }
    await refreshView("tasks");
    return true;
  } catch (error) {
...
async function cancelTask(task: Task) {
  ...
  if (!window.confirm(`确认取消任务“${task.id}”吗？插件将在下一次心跳时停止执行。`)) return false;
  cancellingTaskId.value = task.id;
  errorMessage.value = "";
  try {
    await apiFetch<Task>(`/api/v1/admin/tasks/${encodeURIComponent(task.id)}:cancel`, { method: "POST" });
    await refreshView("tasks");
    return true;
  } catch (error) {
...
async function cancelSchedule(schedule: TaskSchedule) {
  if (schedule.status !== "active" || !window.confirm("确认停用该计划吗？已经生成的任务不会被删除。")) return false;
  cancellingScheduleId.value = schedule.id;
  errorMessage.value = "";
  try {
    await apiFetch<TaskSchedule>(`/api/v1/admin/schedules/${encodeURIComponent(schedule.id)}`, { method: "DELETE" });
    await refreshView("tasks");
    return true;
  } catch (error) {
    errorMessage.value = error instanceof Error ? error.message : "计划停用失败。";
    return false;
  } finally {
    cancellingScheduleId.value = "";
  }
}
async function revokeDevice(device: Device) {
  if (!window.confirm(`确认撤销设备“${device.name}”吗？撤销后该设备将无法继续上传。`)) return;
  revokingDeviceId.value = device.id;
  errorMessage.value = "";
  try {
    await apiFetch(`/api/v1/admin/devices/${encodeURIComponent(device.id)}:revoke`, { method: "POST" });
    await refreshView("devices");
  } catch (error) {
    errorMessage.value = error instanceof Error ? error.message : "设备撤销失败。";
  } finally {
    revokingDeviceId.value = "";
  }
}
async function changePage(kind: keyof typeof pagination, page: number) {
  const state = pagination[kind];
  const target = Math.min(Math.max(page, 1), state.total_pages);
  if (target === state.page && state.total === 0) return;
  state.page = target;
  if (kind === "items" && selectedRun.value) await selectRun(selectedRun.value, { resetPage: false });
  else if (kind === "runs") await refreshView("runs");
  else if (kind === "devices" || kind === "authorizations") await refreshView("devices");
  else if (kind === "tasks" || kind === "schedules") await refreshView("tasks");
  else await refreshView("monitoring");
}
function localMeta<T>(items: T[], state: PageMeta): PageMeta {
  return {
    total: items.length,
    page: state.page,
    page_size: state.page_size,
    total_pages: Math.max(1, Math.ceil(items.length / state.page_size))
  };
}
function formatTime(value?: string) {
  return value ? new Date(value).toLocaleString("zh-CN", { hour12: false }) : "未上报";
}
...
function isDeviceOnline(device: Device) {
  if (device.status !== "active" || !device.last_heartbeat_at) return false;
  return Date.now() - new Date(device.last_heartbeat_at).getTime() < 150_000;
}
